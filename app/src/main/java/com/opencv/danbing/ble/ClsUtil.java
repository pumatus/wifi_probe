...
//        cancelBondProcess(btClass, device);
	...
	public static void setUserParam(Context ctx, String param, String value) {
		SharedPreferences settings = ctx.getSharedPreferences("settingFile",
				MODE_PRIVATE);
	 ...
	 * 获取配置文件
	 */
	public static String getParam(Context ctx, String param) {
		SharedPreferences settings = ctx.getSharedPreferences("settingFile", 0);
	...
	public static void clearSPFile(Context ctx) {
		@SuppressLint("SdCardPath") File file= new File("/data/data/"+ ctx.getPackageName() +"/shared_prefs","settingFile.xml");
		if(file.exists()) {
			file.delete();
		}
		SharedPreferences sp=ctx.getSharedPreferences("settingFile", MODE_PRIVATE);
		if(sp!=null){
			sp.edit().clear().apply();
		}
	}
